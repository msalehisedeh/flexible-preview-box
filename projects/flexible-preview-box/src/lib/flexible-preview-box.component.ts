...
export class FlexiblePreviewBoxComponent implements OnInit {
  ...
  hoverOver(event: any) {
	...
	hoverViewPort(event: any) {
      ...
      const yy = dy - (this.effects.fixedHover ? 1/4*y : y);
      const xx = dx - (this.effects.fixedHover ? 1/4*x : x);
			this.largeImage.nativeElement.style.top = yy + "px";
			this.largeImage.nativeElement.style.left = xx + "px";
			this.largeImage.nativeElement.style.opacity = 1;
      this.onaction.emit({
        action: 'viewport',
        type: event.type,
